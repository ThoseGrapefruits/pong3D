...
(define-syntax (maybe-init stx)
  (syntax-parse stx
    #:datum-literals (<= =)
    [(_ ctx (<= network:expr other:expr ...))
     (with-syntax ([ni (datum->syntax #'ctx 'network-init)])
       #'(ni network))]
    [(_ ctx (= rhs:expr)) #'#f]))
         ...
         (with-syntax
             ([maker
               #`(lambda ()
                   ...
                   (define signal-proc (maybe-init #,stx clause.rhs))
                   ...
                   ...
                   (lambda (in ...)
                     (first-time-fun in ...)))])
           (with-syntax ([ns (datum->syntax stx 'network/s)])
             #`(ns (quote #,num-ins) 
                   (quote #,num-outs)
                   maker)))))]))
...
(define (signal-samples signal n)
  (unless (signal? signal)
    (raise-argument-error 'signal-samples "signal" 0 signal n))
...
(define (signal-nth signal n)
  (unless (signal? signal)
    (raise-argument-error 'signal-nth "signal" 0 signal n))
