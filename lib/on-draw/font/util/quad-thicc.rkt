...
; Positions must be given in counter-clockwise order to show the front as you'd
; expect in (x,y) screenspace.
(: quad-thicc : Pos Pos Pos Pos -> Pict3D)
(define (quad-thicc c1 c2 c3 c4)
  ; We treat the given positions as center positions to make alignment w/
  ; other shapes easier.
  ...
  (combine
   ...
   ; sides (note: order matters here — only one side is visible)
